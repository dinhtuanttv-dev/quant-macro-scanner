...
export function computeTang4(tang3List: Tang3Stock[]): Tang4Stock[] {
  const scored: Tang4Stock[] = tang3List.map((s) => {
    const ta = taSignalPool[s.ticker];
...
export function computeConfluence(
  ...
  const results: ConfluenceStock[] = Array.from(allTickers).map((ticker) => {
    ...
    const t4Score = t4Map[ticker]?.tang4Score ?? 0;
    ...
    return {
      ...
      confluenceBonus,
      finalScore,
      taData: t4Map[ticker]?.taData ?? taSignalPool[ticker] ?? null,
    };
  });
...
export function runFullFunnel(universe: UniverseStock[]) {
